...
| `content` | `content-workbench.html` (Content Wording) | Làm wording được gán, lưu nháp, **gửi Lead Content duyệt**. **Từ 2026-07-06**: nút **"Đề xuất task mới"** (tab Content Tasks của tôi) — tự tạo task `content_initiated` (PIC = chính mình, viết luôn); **1 gate duy nhất** trước khi gửi Media = Lead duyệt nội dung. ⚠ `add-content-self-initiative.sql` |
...
**Từ 2026-07-31**, `lead_media` có workspace riêng **Media Operations** (`media-operations.html`, sidebar "Media Operations") và là **owner chính của Media Order** (Quay / Chụp / Video): kiểm tra + chốt logistics, chốt lịch quay/chụp, gán PIC Quay/Chụp/**Dựng**, tạo Content Script Subtask khi order cần kịch bản, Push Production, theo dõi review/delivery. Media Order **KHÔNG đi qua Content Wording** — Account chỉ hỗ trợ liên hệ requester/client. ⚠ `add-media-operations.sql`
...
| Media Operations | Done | `media-operations.html` + `media-operations.js` (sidebar **"Media Operations"**, role `lead_media`/`admin` vận hành · `account`/`system_supervisor` chỉ xem) — workspace của **Lead Media** cho Media Order (Quay/Chụp/Video). 6 view: Tổng quan (Action Center 10 card) · Media Orders chờ xử lý · Lịch quay/chụp (nhóm theo ngày + filter) · Production Tasks (lọc media/shoot/photo/video/edit) · Review/Delivery · Tất cả. **Media KHÔNG đi Content Wording** — cổng là logistics (ngày/giờ · địa điểm · onsite · dịch vụ · PIC · deadline); Media có kịch bản (TVC/testimonial/interview/VO…) sinh **Content Script Subtask** (`content_tasks.source='media_order'`) → Lead Content duyệt → Lead Media chốt Script Approved → mở khoá Push. Push tạo task Quay/Chụp/Dựng. Routing helper dùng chung `window.MH.routing` (app.js). ⚠ `add-media-operations.sql` |
...
├── media-operations.html Media Operations — workspace Lead Media (logistics/lịch/PIC/script/review)
...
└── assets/
    ...
    ├── media-operations.js   media-operations.html — routing Media, logistics/lịch/PIC, script subtask, push production
...
10. [`clear-demo.sql`](supabase/clear-demo.sql) — utility wipe TOÀN BỘ data trước production test/UAT. **v2 2026-07-15**: cover đủ 17 bảng (thêm notifications/lead_tasks/content_plans/content_tasks/brand_checks…, guard `to_regclass` nên bảng chưa migrate không làm chết script) + xóa file 4 Storage buckets (brief-files/deliverables/plan-files/brand-check-images; GIỮ avatars) — Supabase hosted chặn delete storage.objects qua SQL (`storage.protect_delete` 42501) nên block storage có TRY/CATCH: bị chặn → NOTICE + xóa tay trên Dashboard → Storage. Giữ users/user_role_map/settings.
...
24. [`add-notify-roles-rpc.sql`](supabase/add-notify-roles-rpc.sql) — **RPC `notify_roles` (SECURITY DEFINER)**: fix noti client→staff chết im lặng — producer noti chạy trong phiên CLIENT (order mới, rating, feedback, approve preview/wording, ads order, brand-check) lookup `users` theo role nhưng RLS chỉ cho client đọc chính mình → trả `[]` im lặng → 0 noti insert. RPC lookup + insert notifications hộ trong DB; whitelist CHỈ role nội bộ (client không spam client khác); KHÔNG mở RLS users. Chạy sau `rls.sql` + `add-notifications.sql`. **Kèm Edge Function** [`functions/notify-email/`](supabase/functions/notify-email/index.ts) (optional): email về mail công ty cho ĐÚNG 2 sự kiện — **order mới từ client** + **client hoàn thành rating**. Kích hoạt: Supabase Dashboard → Database → Webhooks → webhook bảng `orders` (INSERT+UPDATE) → Edge Function `notify-email` (function tự lọc event); deploy `supabase functions deploy notify-email --no-verify-jwt` + secrets `RESEND_API_KEY`/`NOTIFY_EMAIL_TO`/`NOTIFY_EMAIL_FROM`/`NOTIFY_EMAIL_SECRET`/`APP_BASE_URL` (provider Resend, free 100 email/ngày).

25. [`add-agreed-deadline-flow.sql`](supabase/add-agreed-deadline-flow.sql) — **P0 Deadline Flow Client↔Account** (2026-07-15): cột `orders.agreed_deadline` + 8 cột proposal (`deadline_proposal_status` none/proposed/accepted/rejected + reason/by/at/responded/note + `deadline_history` jsonb) + CHECK; notifications.type += `deadline_proposed`/`deadline_accepted`/`deadline_rejected`/`deadline_updated`; RPC **`respond_deadline_proposal`** (SECURITY DEFINER — client phản hồi đề xuất, guard đúng requester + status proposed, chỉ đụng cột deadline-response). 3 lớp deadline: `requested_deadline` (gốc Client, KHÔNG ghi đè — audit) · `agreed_deadline` (đã thống nhất) · `internal_deadline` (nội bộ, Client không thấy). Hiển thị Client/Account = **effective = agreed ‖ requested**. Chạy sau `rls.sql` + `add-preview-approval.sql`.

26. [`add-content-task-code.sql`](supabase/add-content-task-code.sql) — **Mã task nội bộ Content**: cột `content_tasks.task_code` (`CT-2026-001`, `CT-2026-002`…) + `SEQUENCE content_task_code_seq` + trigger BEFORE INSERT `set_content_task_code()` sinh mã tự động + backfill task cũ theo thứ tự `created_at` + unique index. **Sinh mã ở DB (không phải frontend)** vì content task được tạo ở 4 chỗ (PIC tự đề xuất · Lead tạo task nội bộ · task con của Plan · tách từ Ads Order) — trigger phủ hết và sequence atomic nên không trùng khi 2 người tạo cùng lúc. Frontend chỉ hiển thị (`ctCode`/`cwbCode`, fallback `CT-<4 ký tự cuối id>` khi chưa migrate) + tìm kiếm theo mã. KHÔNG đụng RLS. Chạy sau `add-content-initiatives.sql`.

27. [`add-media-operations.sql`](supabase/add-media-operations.sql) — **Media Capture Routing + Lead Media Workspace**: `orders` += `media_logistics_status` (pending/checking/need_info/confirmed — cổng THAY THẾ Content Wording cho Media) · `media_schedule_status` · `media_script_required` · `media_script_status` · `media_review_status` · `media_content_type` · `media_script_task_id` · `media_service` · `onsite_contact`/`onsite_phone` · `media_logistics_note` · `production_pic_editor`(+`_user_id`) + 4 CHECK mềm + 3 index; `content_tasks.source` CHECK **+= `media_order`** (Content Script Subtask từ Media Order) + cột `media_content_type`/`script_link`; **RLS `lead_media` CHỈ trên `source='media_order'`** (INSERT/SELECT/UPDATE + comments — KHÔNG mở content task khác cho Lead Media); backfill order Media cũ đã confirmed/pushed → `media_logistics_status='confirmed'`. Chạy sau `rls.sql` + `add-shoot-date.sql` + `add-content-initiatives.sql` + `add-ads-orders.sql` + `add-media-lead-production.sql`. Idempotent.

Seed/utility ngoài migration:
- [`seed-user-role-map.sql`](supabase/seed-user-role-map.sql) — roster nhân sự UAT: upsert `user_role_map` (email công ty + role + TÊN khớp assigned_pic) + block SYNC sửa role/name cho user đã lỡ SSO trước khi seed + query verify ai chưa SSO. Chạy sau `add-google-sso-cbcentres.sql`.

### UAT (2026-07-15)

Folder [`UAT/`](UAT/) phục vụ đợt test nội bộ bằng tài khoản công ty:
- [`UAT_TestPlan.csv`](UAT/UAT_TestPlan.csv) — 78 test case (UTF-8 BOM), 7 nhóm F0 Login → F6 Phân quyền/Cross, import Google Sheets để cả team điền kết quả (Pass/Fail/Blocked + mức độ + ảnh).
- [`UAT_HuongDan.md`](UAT/UAT_HuongDan.md) — pre-flight checklist cho admin (migration pending, SSO, seed roster, chạy clear-demo v2) + hướng dẫn nhân sự (clear site data, cách điền Sheet, mức độ lỗi) + kế hoạch 3 vòng (smoke → kịch bản → free test) + phân vai 6 người.
- [`uat-guide.html`](UAT/uat-guide.html) — bản HTML trực quan của UAT_HuongDan cho nhân sự (self-contained, brand navy/đỏ, inline SVG icons, responsive; mở trực tiếp bằng browser hoặc share link `/UAT/uat-guide.html` khi deploy). Phần admin pre-flight thu gọn trong `<details>`. Sửa nội dung hướng dẫn → sync CẢ `.md` lẫn `.html`.
...
**⚠ Đọc [`CLAUDE.md`](CLAUDE.md) TRƯỚC khi sửa project** — file luật: không đoán tên cột/API/enum (phải grep schema gốc), quy trình 6 bước, cách verify bằng Chrome headless (máy không có Node/Python), luật SQL/RLS/frontend.
...
*Last updated: 2026-07-31 · **Media Capture Routing + Lead Media Workspace (module #16)** — Media Order (Quay/Chụp/Video) bypass Content Wording, cổng mới = logistics; trang `media-operations.html` cho Lead Media (6 view + drawer 10 block + push tạo task Quay/Chụp/Dựng); Media có kịch bản sinh Content Script Subtask (`content_tasks.source='media_order'`) — Lead Content duyệt rồi trả Lead Media chốt Script Approved; routing helper chung `window.MH.routing`; KHÔNG đụng luồng Ads (⚠ chạy `add-media-operations.sql`) · **Content Team Stage D** — tab "Tất cả" hợp nhất 3 nguồn + filter Nguồn, panel Ads trong Cần xử lý, chip source/SLA/next-action phủ toàn workspace, chống đụng tên "Task nội bộ" Media↔Content · **P0 Deadline Flow Client↔Account** — 3 lớp deadline (requested gốc / agreed thống nhất / internal nội bộ), Account đề xuất + lý do → Client Đồng ý / Cần trao đổi lại (RPC `respond_deadline_proposal`), noti 2 chiều 4 type mới, effective deadline (agreed‖requested) áp Database Orders + Client Portal + Calendar, cảnh báo internal_deadline trễ hơn deadline Client (⚠ chạy `add-agreed-deadline-flow.sql`) · **Dashboard Workload DYNAMIC** — Master + Task Dashboard bỏ hết PIC hardcode, build từ users thật + tasks thật (user mới hiện 0/max, nhân sự cũ còn task vẫn hiện), filter PIC dashboard build động, count orders header thật · **UAT reset + test kit** — `clear-demo.sql` v2 full coverage (17 bảng + 4 Storage buckets, guard bảng chưa migrate) · `seed-user-role-map.sql` roster SSO + sync role user lỡ SSO trước seed · folder `UAT/` (TestPlan 78 case CSV + Hướng dẫn 3 vòng test) · **Fix noti client→staff (RPC `notify_roles`)** — mọi noti bắn từ phiên client (order mới, rating, feedback, approve preview/wording, ads, brand-check — 9 chỗ) trước đây chết im lặng vì RLS users chặn client lookup theo role; nay qua RPC SECURITY DEFINER (⚠ chạy `add-notify-roles-rpc.sql`) · **Email notify 2 sự kiện** — Edge Function `notify-email` (Resend + Database Webhook `orders`): mail công ty khi có order mới từ client + khi client hoàn thành rating · **Ads client notify + lead_media order_new** — Ads lifecycle giờ báo client 3 nấc `running`/`completed`/`cancelled` (type `order_status_changed`, gọi trong `adsAdvance`; resume `paused`→`running` không re-notify); order mới từ client notify thêm role `lead_media` (ngang Account) · **Ads Orders** — luồng Client → Content Team (`request.html?type=ads` + `ads-order-form.js` + tab Ads Orders trong content-team + Internal Media Request `ADS-MEDIA-`; prefix `ADS-`/`MEDIA-`/`ADS-MEDIA-`; ⚠ `add-ads-orders.sql`) · Module **Supervisor Planning** (`supervisor-planning.html`+`.js`, bảng `lead_tasks`, role mới `lead_media`, ⚠ `add-supervisor-planning.sql`) · Role `system_supervisor` (Giám sát hệ thống) — monitor read-only toàn hệ thống (⚠ `add-system-supervisor.sql`) · Reports wired LIVE Supabase (module cuối cùng nối DB) + Content Team Workspace (role lead_content + content, content-team.html, ⚠ add-content-team.sql) + Workflow notifications khép kín + bàn giao trong Order drawer (Delivery Log gỡ) + order media Quay/Chụp gộp tách 2 task + Order Form rút gọn theo type + New Orders card*
