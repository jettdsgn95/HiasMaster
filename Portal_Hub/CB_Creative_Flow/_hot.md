...
- **Role `lead_media` ("Lead Media") — Lead nhóm Media cho Supervisor Planning (2026-06-17)**: role MỚI song song `lead_content`. Login redirect → `supervisor-planning.html`. **Chỉ wired surface = trang Kế hoạch** (+ AI Tools/Chatbot mặc định) — nav Production (Master/Task Dashboard/Task Tracker) ẩn qua CSS `:has` như lead_content; `production-board.js` redirect lead_media → supervisor-planning (tránh chạm task sản xuất khi chưa có READONLY guard cho role này). Badge teal `r--lead_media`/`rt--lead_media`; `app.js roleLabel`="Lead Media" + `ROLES`; user-management ROLE_LABEL/defaultPermByRole(manager·production_only·assigned)/2 dropdown. Quyền trên `lead_tasks`: SELECT/UPDATE/DELETE dòng `assigned_lead='lead_media'` (KHÔNG INSERT — chỉ supervisor/admin tạo); UI Lead chỉ đổi `status`. ⚠ chạy `add-supervisor-planning.sql` (users_role_check += lead_media) rồi tạo user Supabase Auth metadata `{name, role:'lead_media'}`. **CSS substring-safe**: "lead_media" KHÔNG chứa admin/account/design/editor/content/client; nhưng data-show-roles của nav "Kế hoạch" chứa "lead_content"⊃"content" → role content reveal nhầm → có override `:has(a[href="supervisor-planning.html"])` ẩn cho content.
