...
- **Bell icon = minimal line SVG theo `notification.type`** (2026-06-01): `app.js` có `NOTIF_ICON_PATHS` (outline Lucide-style, `stroke-width 2`, `currentColor`, 18px, đồng bộ sidebar) + `notifIcon(type)` (→ svg + class màu) + `stripNotifEmoji(title)` (bỏ emoji dẫn đầu trong title cũ). Dropdown item layout `[span.mh-notif-item-ico][div.mh-notif-item-body]`. Màu mặc định `var(--text-muted)`, `is-accent`=navy cho order_new/task_assigned, `is-danger`=red cho order_cancelled/order_needinfo. **KHÔNG thêm emoji mới vào title producer** — icon do consumer render theo type; nếu cần icon mới chỉ thêm key vào `NOTIF_ICON_PATHS`.
...
- **Client notification panel cũng dùng cùng icon (2026-06-02)**: `client-dashboard.js renderNotifications` (tab "Thông báo" của Client Portal) là consumer RIÊNG, không qua app.js bell. Đã copy `NOTIF_ICON_PATHS` + `notifIcon(rawType)` + `stripNotifEmoji()` y hệt app.js, key theo `n.raw_type` (Supabase type) để icon + màu **đồng bộ 100%** với internal accounts. Item đổi `.notif-dot` → `.notif-ico` (CSS inline trong `client-dashboard.html`). **Nếu sửa icon/màu ở app.js phải sửa song song ở client-dashboard.js** (2 nơi giữ map riêng, chưa tách shared module).
