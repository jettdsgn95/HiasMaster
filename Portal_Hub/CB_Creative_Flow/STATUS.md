...
>
> *Last updated: 2026-05-23 · Project state: Production-ready beta · Phase 1+2 LIVE · Realtime notifications · Demo data cleared · Cancel-order modal · **3 Dashboards fully wired** (Master separated combined / Orders Dashboard 13-KPI Client lifecycle / Task Dashboard 17-KPI Internal workload) · UI naming consistency (Modules 1-5) · **Client notifications sync LIVE** · Railway deploy LIVE ✓*
...
| Date | Module | Action |
|---|---|---|
| 2026-05-23 | Header compact redesign + lang toggle | **Header top-right compact theo brief UI** (4 circular elements 36px: VN pill / Theme circle / Bell circle / Avatar circle). Bỏ pill switch theme cũ (48×26 với thumb translateX) chuyển sang icon-only circular đồng nhất bell. Profile chip thu gọn chỉ avatar 30px (ẩn name + role badge + chevron), click vẫn mở dropdown cũ. Thêm `assets/i18n.js` lightweight lang module (persist `localStorage['mh-lang']` 'vi'/'en', set `<html lang>`, expose `MH.i18n.{getLang,setLang,t,apply}`). Translation phase 1 chỉ cover header chrome (profile menu 3 items + 3 aria labels) qua `data-i18n` / `data-i18n-aria` attrs — page content vẫn Vietnamese-first, full translation defer. CSS thêm `.lang-pill` + rewrite `.icon-btn`/`.theme-toggle-switch`. Bulk-update 17 HTML page: lang pill trước theme toggle, script i18n.js trước app.js, data-i18n cho menu + aria |
